...
resource "null_resource" "cassandra" {
  ...
  triggers = {
    triggers = element(var.vm_ids, count.index)
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-playbook -u ${var.user_name} -i ${element(var.host_list, count.index)}, cassandra-server.yml --extra-vars=CASSANDRA_MEMTABLE_THRESHOLD=${var.memtable_threshold} --extra-vars=CASSANDRA_SEEDS=${join(",", var.host_seed_list)} --extra-vars=enable_tdagent=${var.enable_tdagent ? 1 : 0} --extra-vars=CASSANDRA_STATE=${var.start_on_initial_boot ? "started" : "stopped"}",
    ]
  }
}
