...
resource "aws_ebs_volume" "monitor_log_volume" {
  count = local.enable_tdagent && local.enable_log_volume ? local.resource_count : 0

  availability_zone = module.monitor_cluster.availability_zone[count.index]
  size              = local.log_volume_size
  type              = local.log_volume_type
...
resource "null_resource" "volume_data" {
  ...
  connection {
    ...
    host         = element(module.monitor_cluster.private_ip, count.index)
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo sh -c 'echo export LOG_STORE=${element(aws_ebs_volume.monitor_log_volume.*.id, count.index)} > /etc/profile.d/volumes.sh'",
    ]
  }
}
...
resource "aws_route53_record" "monitor-dns" {
  ...
  type    = "A"
  ttl     = "300"
  records = [element(module.monitor_cluster.private_ip, count.index)]
}
