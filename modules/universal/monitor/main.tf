...
resource "null_resource" "docker_install" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-playbook -u ${var.user_name} -i ${var.host_list[count.index]}, monitor-server.yml --timeout 60 -e enable_tdagent=${var.enable_tdagent ? 1 : 0} -e log_retention_period_days=${var.log_retention_period_days} -e log_archive_storage_info=${var.log_archive_storage_info}",
    ]
  }
}
