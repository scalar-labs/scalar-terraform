...
output "inventory_ini" {
  value = <<EOF
[scalardl_blue]
%{for f in azurerm_private_dns_a_record.scalardl_blue_dns.*.fqdn~}
${f}
%{endfor}
[scalardl_green]
%{for f in azurerm_private_dns_a_record.scalardl_green_dns.*.fqdn~}
${f}
%{endfor}
[envoy]
%{for f in azurerm_private_dns_a_record.envoy_dns.*.fqdn~}
${f}
%{endfor}
